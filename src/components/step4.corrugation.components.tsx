...
import { corrugationParams } from "../assets/urci-params/params.data";
...
const CorrugationComponent: React.FC<Props> = ({
  onDataUpdate,
  meassuredLength,
  meassuredWidth,
  meassuredArea,
  ...
  useEffect(() => {
    if (length && width) {
      ...
      if (density) {
        const y = searchY(corrugationParams, cat, density);
        if (y !== null) {
          setDv(y);
          onDataUpdate(4, { deduct_value: y });
        }
      }
    }
      ...
      <div className="w-full">
        <div className="flex gap-2 sm:gap-3 items-center">
          <div className="w-7 h-7 sm:w-8 sm:h-8 bg-crayola rounded-full flex items-center justify-center">
            <p className="font-semibold text-lg md:text-xl text-alabaster">4</p>
          </div>
...
export default CorrugationComponent;
